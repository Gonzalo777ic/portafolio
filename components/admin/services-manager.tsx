...
function ServiceEditor({ service }: { service: Service }) {
  ...
  return (
    <li className="space-y-4 rounded-2xl border border-white/10 bg-white/5 p-5">
      <div className="flex items-center gap-3">
        <ServiceGlyph name={icon} color={iconColor} className="h-6 w-6" />
    ...
    <div className="space-y-6">
      {services.length === 0 ? (
        <p className="rounded-xl border border-dashed border-white/15 px-4 py-6 text-center text-sm text-neutral-500">
          No hay servicios en la base. Añade el primero abajo.
        </p>
      ) : (
